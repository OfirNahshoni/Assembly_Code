# Question 7 - Project 1 Lab CS
# Purpose: Program that swap between 2 followed bytes in each
# word, in array A
...
add $t0,$a2,$0 # Address of the first even byte in array A
addi $t1,$a2,1 # Address of the first odd byte in array A
...
lb $t2,0($t0) # Load Byte: load byte in even index
lb $t3,0($t1) # Load Byte: load byte in odd index
sb $t3,0($t0) # Store Byte: store odd byte in even index
sb $t2,0($t1) # Store Byte: store byte byte in odd index
addi $t0,$t0,2 # Promote current even to the next even
addi $t1,$t1,2 # Promote current odd to the next odd
j loop1 # Jump to label 'swap_even_odd_followed'
